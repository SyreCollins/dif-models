...
# This is the integrated run script for the model.
# It reads input CSV files, runs the Analytics_Model for each product,
# concatenates the results, and writes the output to a CSV.
# For the API, we wrap this process in a function that returns the results as JSON.

def run_full_model():
    project_datas = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/ipem_2025/data_inputs/project_data.csv")
    multipliers = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/ipem_2025/data_inputs/sectorwise_multipliers.csv")
    #results = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/ipem_2025/results/results_all.csv")  # Not used in API run
    ...
    # Instead of writing to CSV, we return the results as a dictionary
    return results_all.to_dict(orient="records")

########################################################INTEGRATED PROJECT ECONOMICS MODEL ENDS######################################################################
...
class MicroEconomicInput(BaseModel):
    data: Dict[str, Any]
    plant_mode: str
    fund_mode: str
    opex_mode: str
...
class MacroEconomicInput(BaseModel):
    data: Dict[str, Any]
    multiplier: Dict[str, Any]  # or a nested structure
    location: str
    plant_mode: str
    fund_mode: str
    opex_mode: str
...
@app.get("/")
def read_root():
    return {"message": "This is the IPEM Model API. Use /chemprocess, /microeconomic, /macroeconomic, /analytics, or /run_model endpoints."}

@app.post("/chemprocess")
def api_chemprocess(input_data: DataInput):
    try:
        result = ChemProcess_Model(input_data.data)
        return {"result": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/microeconomic")
def api_microeconomic(input_data: MicroEconomicInput):
    try:
        result = MicroEconomic_Model(input_data.data, input_data.plant_mode, input_data.fund_mode, input_data.opex_mode, input_data.carbon_value)
        return {"result": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/macroeconomic")
def api_macroeconomic(input_data: MacroEconomicInput):
    try:
        result = MacroEconomic_Model(input_data.multiplier, input_data.data, input_data.location, input_data.plant_mode, input_data.fund_mode, input_data.opex_mode, input_data.carbon_value)
        return {"result": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/analytics")
def api_analytics(input_data: AnalyticsInput):
    try:
        # For analytics, assuming that the required CSV files are at the specified paths.
        project_datas = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/ipem_2025/data_inputs/project_data.csv")
        multipliers = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/ipem_2025/data_inputs/sectorwise_multipliers.csv")
        result = Analytics_Model(multipliers, project_datas, input_data.location, input_data.product,
                                 input_data.plant_mode, input_data.fund_mode, input_data.opex_mode, input_data.carbon_value)
